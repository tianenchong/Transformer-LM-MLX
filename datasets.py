...
def load_dataset(dataname, save_dir):
    if dataname == "enwik8":
        return enwik8(save_dir=save_dir)
    elif dataname == "ptb":
        return ptb(save_dir=save_dir)
    elif dataname == "wikitext2":
        return wikitext(dataset="2",save_dir=save_dir)
    else:
        return wikitext(dataset="103",save_dir=save_dir)
...
if __name__ == "__main__":
    save_dir = "/Volumes/RAMDisk/transformer-lm"

    vocab, train, val, test = enwik8(save_dir=save_dir)
    assert len(vocab) == 205, "enwik8: Wrong vocab size"

    vocab, train, val, test = ptb(save_dir=save_dir)
    assert len(vocab) == 10000, "PTB: Wrong vocab size"

    vocab, train, val, test = wikitext(save_dir=save_dir)
